...
// 	if (ad_turn == 0)		//V_OUT
// 	{
// 		// $ adcc enable, P_check_temprature; //下次温度通道
...
// 		// (不用除以采样次数，因为两边都累加了N次)
// 		//math_dword = ad_sum * 120;     // Vbg=1.2V，放大100倍，120是1.2*100
// 		math_eword = ad_sum;
// 		math_byte = 120;
// 		Math_Eword_Mul_Byte();
...
// 		math_eword = ad_sum_bandgap;
// 		Math_Dword_Div_Eword();
// 		V_temp = math_dword; // 单位：0.01V（10mV），你用作比值或查表即可
...
// 		math_word = V_temp;    // 0.01V单位的V_temp
// 		math_eword = 6800;     // 6.8k欧姆
// 		EWord_Mul_Word(); // 结果在math_eword
...
//         ad_sum_bandgap = ad_sum;                // 本轮采集到的是BANDGAP结果

//         // 可选：处理Vout参考（如果你有需求，这里是bandgap的AD和）
//     }
